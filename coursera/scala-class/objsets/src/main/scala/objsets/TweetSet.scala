...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = ascendingByRetweet0(this)
  
  def ascendingByRetweet0(tweetSet: TweetSet): Trending =
    if (tweetSet isEmpty) new EmptyTrending
    else {
      val min = tweetSet.findMin
      new NonEmptyTrending(min, ascendingByRetweet0(tweetSet.remove(min)))
    }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    right.filter0(p, left.filter0(p, {
      if (p(elem)) accu.incl(elem) else accu
    }))
  }
  // The following methods are provided for you, and do not have to be changed
  // -------------------------------------------------------------------------
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.exists(tweet.text.contains(_)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.exists(tweet.text.contains(_)))
